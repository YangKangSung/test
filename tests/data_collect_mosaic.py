...
class MosaicScraper:
    ...
    def extract_post_info(self, post_elem, base_url: str) -> Optional[PostData]:
        """개별 게시물 정보 추출"""
        try:
            # 게시물 제목
            title_elem = post_elem.select_one('.title a')
            title = self.clean_text(title_elem.get_text()) if title_elem else "제목 없음"

            # 게시물 URL
            post_url = urljoin(base_url, title_elem['href']) if title_elem else ""

            # 작성자
            author_elem = post_elem.select_one('.author')
            author = self.clean_text(author_elem.get_text()) if author_elem else "작성자 없음"

            # 작성일
            date_elem = post_elem.select_one('.date')
            post_date = self.clean_text(date_elem.get_text()) if date_elem else "날짜 없음"

            # 조회수
            view_elem = post_elem.select_one('.view-count')
            view_count = int(self.clean_text(view_elem.get_text())) if view_elem else None

            # 좋아요 수
            like_elem = post_elem.select_one('.like-count')
            like_count = int(self.clean_text(like_elem.get_text())) if like_elem else None

            # 댓글 수
            comment_elem = post_elem.select_one('.comment-count')
            comment_count = int(self.clean_text(comment_elem.get_text())) if comment_elem else None

            # 카테고리
            category_elem = post_elem.select_one('.category')
            category = self.clean_text(category_elem.get_text()) if category_elem else None

            # 태그
            tag_elems = post_elem.select('.tags a')
            tags = [self.clean_text(tag.get_text()) for tag in tag_elems] if tag_elems else []
    ...
    def enhance_post_data(self, post: PostData) -> PostData:
        ...
        soup = self.fetch_page(post.post_url)
        if not soup:
            return post
        ...
        try:
            ...
            if content_elem:
                post.content = self.clean_text(content_elem.get_text())
    ...
    def scrape_post_list(self, url: str) -> List[PostData]:
        ...
        posts = []
        post_elements = soup.select('.post-list .post-item')
        ...
        for elem in post_elements:
            post_data = self.extract_post_info(elem, self.base_url)
            if post_data:
...
def main():
    ...
    logger.info("삼성 모자이크 커뮤니티 페이지 스크래핑 시작")
